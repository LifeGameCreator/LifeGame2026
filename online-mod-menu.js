    ...
    if (tab === "phone") {
      ...
      return `
        <section class="online-mod-section-grid two">
          ...
          <article class="online-mod-card"><div class="online-mod-card-title"><span>♥</span><div><small>FINDER.KL</small><h3>Dating-Profil & Online-Status</h3></div></div><div class="online-mod-data-list"><span><small>Profil</small><b>${save.finder?.accountCreated ? "Erstellt" : "Nicht erstellt"}</b></span><span><small>Status</small><b>${save.finder?.online ? "Online" : "Offline"}</b></span><span><small>Name</small><b>${esc(save.finder?.profileName || "–")}</b></span><span><small>Alter</small><b>${num(save.finder?.profileAge || 0) || "–"}</b></span><span><small>Geschlecht</small><b>${save.finder?.profileGender === "female" ? "Frau" : save.finder?.profileGender === "male" ? "Mann" : "–"}</b></span><span><small>Matches</small><b>${safeArray(save.finder?.matches).length}</b></span></div></article>
        </section>`;
    }
