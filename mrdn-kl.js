(() => {
  ...
  const AM_VERSION = "2026-07-24-mrdn-lobby-endgame-v7";
  ...
  function amNormalizeLocalGame(game) {
    ...
    game.players = game.players.slice(0, 4).map((player, index) => ({
      ...
      points: Math.max(0, Math.min(99, Number(player?.points || 0))),
  ...
  function amGameOverlayHtml(game) {
    ...
    return `<section class="am-game-shell" style="--turn-color:${currentColor.hex}">
      <header class="am-game-head">
        ...
        <div class="am-head-actions"><button data-am-fullscreen>${fullscreenText}</button>${game.online ? `<button data-am-copy-code>Code kopieren</button>` : `<button class="am-end-local" data-am-end-local>Spiel beenden</button>`}<button data-am-minimize>Minimieren</button></div>
      </header>
      ...
      ${game.status === "finished" ? `<div class="am-winner-panel"><span>🏆</span><h2>${amEscape(game.winnerName)} gewinnt MRDN.KL</h2><p>Alle vier Figuren sind im Ziel.</p><div>${game.online && amIsHost() ? `<button data-am-rematch>Revanche starten</button>` : !game.online ? `<button data-am-local-rematch>Noch eine Runde</button>` : ""}<button data-am-minimize>Zur App</button></div></div>` : ""}
  ...
  function amBindApp(shell) {
    if (!shell) return;
    if (shell.dataset.amBoundVersion === AM_VERSION) return;
    shell.dataset.amBoundVersion = AM_VERSION;
    ...
    shell.querySelector("[data-am-start-local]")?.addEventListener("click", () => amStartLocal(shell).catch((error) => amToast(error?.message || error)));
  ...
  document.addEventListener("click", (event) => {
    ...
    if (!button) return;
    const count = Number(button.closest(".device-shell")?.querySelector("[data-am-local-count]")?.value || 4);
    setTimeout(() => {
      if (!amRuntime.overlay?.classList.contains("show")) {
        console.warn("MRDN.KL: regulärer Start blieb aus, Fallback wird verwendet.");
        window.AergerMenschKL.openBotParty(count);
      }
    }, 350);
  }, true);
